...
#' Fitting Hierarchical Dyadic Factor Models
#'
...
scriptHier <- function(
  ...
  if(scaleset == "FF"){
    ...
    if(any(constr_dy_meas == "loadings")){
      xloads1 <- loads(dvn, lvar = lvar, lvname = lvname, partner="1", type = "equated", group_n = group_n, constr_group_loadings = constr_group_loadings)
      xloads2 <- loads(dvn, lvar = lvar, lvname = lvname, partner="2", type = "equated", group_n = group_n, constr_group_loadings = constr_group_loadings)
    }
    else{
      xloads1 <- loads(dvn, lvar = lvar, lvname = lvname, partner="1", type = "free", group_n = group_n, constr_group_loadings = constr_group_loadings)
      xloads2 <- loads(dvn, lvar = lvar, lvname = lvname, partner="2", type = "free", group_n = group_n, constr_group_loadings = constr_group_loadings)
    }

    #common fate loadings
    hierloads <- cfloads(dvn, lvname = lvname, type = "equated", group_n = group_n, constr_group_loadings = constr_group_loadings)
    ...
    if(any(constr_dy_struct == "variances")){
      hierxvar <- cfvars(lvname = lvname, type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar1 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "1", type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances, constr_group_loadings = constr_group_loadings)
      xvar2 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "2", type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances, constr_group_loadings = constr_group_loadings)
    }
    else if(!any(constr_dy_struct == "variances") & any(constr_dy_meas == "loadings") & scaleset == "FF"){
      hierxvar <- cfvars(lvname = lvname, type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar1 = lvars(dvn, lvar = lvar, lvname = lvname, partner = "1", type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances, constr_group_loadings = constr_group_loadings)
      xvar2 = lvars(dvn, lvar = lvar, lvname = lvname, partner = "2", type = "free", group_n = group_n, constr_group_variances = constr_group_variances, constr_group_loadings = constr_group_loadings)
    }
    else{
      hierxvar <- cfvars(lvname = lvname, type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar1 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "1", type = "fixed", group_n = group_n, constr_group_variances = constr_group_variances, constr_group_loadings = constr_group_loadings)
    ...
    #hierarchical mean
    xmean <- cfmeans(lvname = lvname, type = "fixed", group_n = group_n, constr_group_means = constr_group_means)

    #latent (partner) means
    if(any(constr_dy_struct == "means")){
      xmean1 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="1", type = "equated_ff", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
      xmean2 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="2", type = "equated", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
    }

    else if(!any(constr_dy_struct == "means") & any(constr_dy_meas == "intercepts") & scaleset == "FF"){
      xmean1 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="1", type = "fixed", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
      xmean2 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="2", type = "free", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
    }

    else{
      xmean1 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="1", type = "fixed", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
      xmean2 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="2", type = "fixed", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
  ...
  if(scaleset == "MV"){
    ...
    #loadings
    if(any(constr_dy_meas == "loadings")){
      xloads1 <- loads(dvn, lvar = lvar, lvname = lvname, partner="1", type = "equated_mv", group_n = group_n, constr_group_loadings = constr_group_loadings)
      xloads2 <- loads(dvn, lvar = lvar, lvname = lvname, partner="2", type = "equated", group_n = group_n, constr_group_loadings = constr_group_loadings)
    }
    else{
      xloads1 <- loads(dvn, lvar = lvar, lvname = lvname, partner="1", type = "fixed", group_n = group_n, constr_group_loadings = constr_group_loadings)
      xloads2 <- loads(dvn, lvar = lvar, lvname = lvname, partner="2", type = "fixed", group_n = group_n, constr_group_loadings = constr_group_loadings)
    }

    #common fate loadings
    hierloads <- cfloads(dvn, lvname = lvname, type = "fixed", group_n = group_n, constr_group_loadings = constr_group_loadings)
    ...
    #latent variances
    if(any(constr_dy_struct == "variances")){
      hierxvar <- cfvars(lvname = lvname, type = "free", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar1 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "1", type = "equated", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar2 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "2", type = "equated", group_n = group_n, constr_group_variances = constr_group_variances)
    }
    else{
      hierxvar <- cfvars(lvname = lvname, type = "free", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar1 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "1", type = "free", group_n = group_n, constr_group_variances = constr_group_variances)
      xvar2 <- lvars(dvn, lvar = lvar, lvname = lvname, partner = "2", type = "free", group_n = group_n, constr_group_variances = constr_group_variances)
    }
    ...
    #latent means
    if(any(constr_dy_struct == "means")){
      xmean1 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="1", type = "equated", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
      xmean2 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="2", type = "equated", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
    }
    else{
      xmean1 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="1", type = "free", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
      xmean2 <- lmeans(dvn, lvar = lvar, lvname = lvname, partner="2", type = "free", group_n = group_n, constr_group_means = constr_group_means, constr_group_intercepts = constr_group_intercepts)
    }
