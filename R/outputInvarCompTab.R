...
#'
#' @return A list. More specifically, a [tibble()]---if `gtTab = FALSE` (default)---or `gt::gt()` object---if `gtTab = TRUE`---of model fit statistics for each model, as well as the difference in fit statistics between each model and the previous model
#' 
#' @details
