...
scriptAPIM <- function(dvn, scaleset = "FF",
  ...
  # Validate lvxname argument
  if (missing(lvxname) || is.null(lvxname)) {
    stop("The `lvxname` argument is required and cannot be NULL.")
  }
  if (!is.character(lvxname)) {
    stop("The `lvxname` argument must be a character string.")
  }

  # Validate lvyname argument
  if (missing(lvyname) || is.null(lvyname)) {
    stop("The `lvyname` argument is required and cannot be NULL.")
  }
  if (!is.character(lvyname)) {
    stop("The `lvyname` argument must be a character string.")
  }

  # Validate est_k argument
  if (!is.logical(est_k)) {
    stop("The `est_k` argument must be a logical value (TRUE or FALSE).")
  }

  # Validate includeMeanStruct argument
  if (!is.logical(includeMeanStruct)) {
    stop("The `includeMeanStruct` argument must be a logical value (TRUE or FALSE).")
  }
