...
scriptMIM <- function(dvn, scaleset = "FF",
  ...
  if (lifecycle::is_present(model)) {
    lifecycle::deprecate_stop(
      when = "1.0.0",
      what = I('The argument "scriptMIM(model)"'),
      with = I('"scriptMIM(constr_dy_x_meas) and/or scriptMIM(constr_dy_y_meas)"')
    )
  }
  ...
  if (lifecycle::is_present(equate)) {
    lifecycle::deprecate_stop(
      when = "1.0.0",
      what = I('The argument "scriptMIM(equate)"'),
  ...
  if(any(constr_dy_xy_struct == "partners"|constr_dy_xy_struct == "all")){
    ...
    partners <- lregs(dvn, param = "mim_part", lvxname, lvyname, type = "zero")
  }else{
