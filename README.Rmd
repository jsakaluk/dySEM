...
The `dySEM` package helps automate the process of scripting, fitting, and reporting on latent models of dyadic data via [`lavaan`](https://lavaan.ugent.be). The package was initially developed and used in the course of the research described in Sakaluk, Fisher, and  Kilshaw (2021), and has since undergone considerable expansion. Version 1.4.2 was released as part of the package's publication in the 
[Journal of Open Source Software](https://joss.theoj.org/papers/10.21105/joss.10115) in 2026.

```{r package-stats, echo = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
# Source helper functions to count package statistics
# Handle case where package_summary.R might not be available (e.g., during R CMD check)
package_summary_path <- "package_summary.R"
if (file.exists(package_summary_path)) {
  source(package_summary_path)
  ...
  # Fallback values if package_summary.R is not available
  # These should be updated manually if package_summary.R is unavailable
  n_total_funcs <- 68
  n_exported_funcs <- 25
  n_test_cases <- 468
}
...
`dySEM` currently contains **`r n_total_funcs`** functions, of which **`r n_exported_funcs`** are user-facing (i.e., exported), which are covered by **`r n_test_cases`** unit tests.
...
* Multiple open-access datasets for learning, practicing, and teaching about dyadic SEM
