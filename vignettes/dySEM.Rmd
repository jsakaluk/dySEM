...
**TODO**: MENTION REPRODUCIBLE WORKFLOWS BEING NECESSARY FOR MANY BENEFITS
...
1. **Output** statisitcal table(s) and or visualization(s) 

You might also use **optional** `dySEM` functions after Step 3 to calculate/output some additional information. 

These families of functions--***Scrapers***, ***Scripters***, and ***Outputters***--are listed and described in the Reference (LINK)

We now demonstrate a typical `dySEM` workflow, using the built-in [`DRES`](https://github.com/jsakaluk/dySEM/blob/master/R/DRES-data%20copy.R) **data** (Raposso & Muise, 2020), in order to perform dyadic confirmatory factor analysis (CFA).

## 1. Import and Wrangle Data

We will use a subset of  [`DRES`](https://github.com/jsakaluk/dySEM/blob/master/R/DRES-data%20copy.R), consisting of 121 dyadic couples' ratings of relationship quality on 9 of the [PRQC](https://journals.sagepub.com/doi/abs/10.1177/0146167200265007) indicators (1 = *not at all*, 7 = *extremely*; all indicators positively keyed). Structural equation modeling (SEM) programs like `lavaan` require dyadic data to be in *dyad structure dataset*, whereby each row contains the data for one dyad, with separate columns for each observation (in this case, indicator variables of latent relationship quality) made for each member of the dyad. 

**TODO**: Mention pivot_ family for data transformation needs

However, like many real-world analytic contexts, [`DRES`](https://github.com/jsakaluk/dySEM/blob/master/R/DRES-data%20copy.R) contains a number of other variables that we aren't interested in modeling at this time (specifically, 5 indicators of sexual satisfaction from the [GMSEX](https://www.tandfonline.com/doi/abs/10.1080/00224490509552264) for each dyad member). This wil not be a problem for `dySEM`. 
...
The `scrapeVarCross` function is your `dySEM` scraper for cross-sectional dyadic data. It can acomodate scraping indicators for models featuring one latent variable (e.g., as in our dyadic CFA), as well as bivariate latent variable models, such as the Actor-Partner Interdependence Model (APIM). We cover scraping and scripting of these bivariate models in other vignettes.
...
* distinguishing **P**artner charcters of "1" and "2"
...
Each *Scripter* function is a wrapper for a series of *Helper* functions (see [`cfa_funs`](https://github.com/jsakaluk/dySEM/blob/master/R/cfa_funs.R) if you are interested) that snatch the information about the indicators they need from a saved `dvn` object and combine it with other text to write the `lavaan` syntax for a particular part of the measurement (e.g., factor loadings, item intercepts) or structural (e.g., regression slopes, factor means) portion of your model. 
...
1. the kind of parameter equality constraints that you wish to be imposed (if any), such as those corresponding to particular levels of measurement invariance (e.g., "loading"), or even a fully "indistiguishable" model (i.e., in which all measurement and structural parameters are constrained to equality between partners)

If you plan on scripting multiple models, I recommend that you name the output of `Scripters` to include information about the  latent variable's name (from 2.) and model (from 3.). For example, if we were to use `scriptCFA` to generate scripts for a configurally invariant CFA of the PRQC items we scraped, we could specify
...
**TODO**: Show image of directory + file and .txt
...
The modeling efficiency and accuracy gained via `dySEM`'s automated scripting may already be apparent, but becomes painfully obvious once you leverage `dySEM` to quickly script a sequence of competing models (e.g., from configural invariance CFA --> fully indistinguisbale CFA)

```{r scriptsequence}
qual.load.script <- scriptCFA(dvn, lvname = "Quality", model = "loading")

qual.int.script <- scriptCFA(dvn, lvname = "Quality", model = "intercept")

qual.res.script <- scriptCFA(dvn, lvname = "Quality", model = "residual")

qual.ind.script <- scriptCFA(dvn, lvname = "Quality", model = "indistinguishable")
```
...
qual.ind.fit <- lavaan::cfa(qual.ind.script, data = DRES, std.lv = FALSE, auto.fix.first= FALSE, meanstructure = TRUE)
```
...
```{r anova}
anova(qual.load.fit, qual.config.fit)
```
...
`dySEM` also contains functionality to help you quickly, correctly, and reproducibly generate output from your fitted model(s), in the forms of path diagrams and/or tables of statistical values. Path diagram creation is supported via the [`semPlot`](https://cran.r-project.org/web/packages/semPlot/index.html) package's `semPaths` function, whereas table creation is currently supported by the [`sjPlot`](https://strengejacke.github.io/sjPlot/) package's `tab_df` function, though users should be aware that [we are considering](https://github.com/jsakaluk/dySEM/issues/28) a move to the [`gt`](https://gt.rstudio.com) package for improved tabling capacity.

The `dyOutput` function is currenty `dySEM`'s all-purpose outputting function. 
...
Whatever options the useR specifies, `dyOutput` is typically run without assigning its output to an object, as it's mostly to facilitate statistical reporting for scientific articles: 

**TODO**: CORRECT CODE GOES HERE
