...
- having a statistical framework (SEM) that allows you to interrogate other auxiliary assumptions that underly your model and any comparisons within it that you wish to make

The specifics of the benefits (i.e., to what degree they help/hinder, and under what methodological circumtances) of using a Latent APIM vs. other approaches are not well known, and are the current target of study for Eric Tu's comps paper. While this research is ongoing, this vignette should serve as a reasonable starting tutorial for those wishing to experiment with applying latent APIMs. 
...
apim.script.config <-  scriptAPIM(dvn, #the list we just created from scrapeVarCross
                                  ...
                                  model = "configural", #what measurement constraints (if any) to impose
                                  equate = "none", #what effects do you want to constrain if any)? 
                                  k = TRUE,#want k-parameter? (optional, but nice)
...
scriptAPIM() also enables users to impose constraints of "**structural indistinguishability**" (i.e., estimating one actor and/or partner effect for both partners). However, the statistical conclusion validity of comparing models with these kinds of constraints to models in which actor and/or partner effects are freely estimated (as in the previous version) depends, in part, on having ensured dyadic measurement invariance, particulalry for the loadings (other comparisons require other forms of invariance to be met). 

scriptAPIM() will allow you to script models imposing structural constraints (via the `equate =` argument), regardless of what level of measurement invariance is imposed (if any) (via the `model = ` argument). However, the function will provide a warning that this equating can only be validly tested when loadings are invariant.  

```{r scripting_warn}

apim.script.config.eq.all <-  scriptAPIM(dvn, #the list we just created from scrapeVarCross
                                  lvxname = "Sat", #arbitrary name for LV X
                                  lvyname = "Com", #arbitrary name for LV Y
                                  model = "configural", #what measurement constraints (if any) to impose
                                  equate = "all_effects", #what effects do you want to constrain if any)? 
                                  k = TRUE,#want k-parameter? (optional, but nice)
                                  writescript = FALSE) #want script saved to directory? (e.g., for OSF?)

```

One can therefore impose the corresponding level of invariance via the `model = ` argument (at minimum `model = loading` is required, but any models that are even more constrained, e.g., `model = residual` would also be acceptable)-- the warning will go away--before proceeding with model fitting and inspecting.

```{r scripting_loading}

apim.script.loads.eq.all <-  scriptAPIM(dvn, #the list we just created from scrapeVarCross
                                  lvxname = "Sat", #arbitrary name for LV X
                                  lvyname = "Com", #arbitrary name for LV Y
                                  model = "loading", #what measurement constraints (if any) to impose
                                  equate = "all_effects", #what effects do you want to constrain if any)? 
                                  k = TRUE,#want k-parameter? (optional, but nice)
                                  writescript = FALSE) #want script saved to directory? (e.g., for OSF?)

apim.fit.loads.eq.all <- cfa(apim.script.loads.eq.all, 
                               data = commitmentQ,
                               std.lv = FALSE, 
                               auto.fix.first= FALSE, 
                               meanstructure = TRUE)
...
Users should note that structural constraints will have downstream impacts on the computation of paramater *k* if it is requested (`k = TRUE`). That is, two *k*'s will be returned if either actor and/or partner effects are uniquely estimated, but only one *k* will be returned if both actor and partner effects are constrained to equivalency (as in the example above). 
